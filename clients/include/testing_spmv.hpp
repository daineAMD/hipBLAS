...
hipblasStatus_t testing_spmv(const Arguments& argus)
{
    ...
    host_vector<T> hy_cpu(M * incx);
    host_vector<T> hy_host(M * incx);
    host_vector<T> hy_device(M * incx);
    ...
    if(argus.unit_check || argus.norm_check)
    {
        ...
        if(argus.unit_check)
        {
            unit_check_general<T>(1, M, incx, hy_cpu, hy_host);
            unit_check_general<T>(1, M, incx, hy_cpu, hy_device);
        }
    ...
    if(argus.timing)
    {
        ...
        ArgumentModel<e_M, e_alpha, e_lda, e_incx, e_beta, e_incy>{}.log_args<T>(
            std::cout,
            argus,
            gpu_time_used,
