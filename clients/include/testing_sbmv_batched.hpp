...
template <typename T>
hipblasStatus_t testing_sbmv_batched(const Arguments& argus)
{
    ...
    if(argus.unit_check || argus.norm_check)
    {
        ...
        if(argus.unit_check)
        {
            unit_check_general<T>(1, M, batch_count, incx, hy_cpu, hy_host);
            unit_check_general<T>(1, M, batch_count, incx, hy_cpu, hy_device);
        }
        if(argus.norm_check)
        {
            hipblas_error_host
    ...
    if(argus.timing)
    {
        ...
        ArgumentModel<e_M, e_K, e_alpha, e_lda, e_incx, e_beta, e_incy, e_batch_count>{}
            .log_args<T>(std::cout,
                         argus,
                         gpu_time_used,
